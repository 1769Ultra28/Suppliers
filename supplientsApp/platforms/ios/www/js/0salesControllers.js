...
.controller('ClientsCtrl', function($scope, $http) {
	...
	$http.get('http://cssc.mine.nu:15001/Clients').success(function(jsonClients){ 
		$scope.clients = (jsonClients);
	});
	//$scope.clients=clientsService.all();
	// alert('Ctrl Clients');
})

.controller('ClientDetailCtrl', function($scope, $stateParams, $timeout, $q, $ionicPopup, $http) {
	$scope.client = {};
	$http.get('http://cssc.mine.nu:15001/Clients').success(function(jsonClients){ 
		$scope.clients = (jsonClients);
		$scope.client = $scope.clients[$stateParams.clientId];
	});
